 ...
 * @retval None
 */
tBleStatus setConnectable(void)
{
    tBleStatus ret;
    ...
    /* BLE MAC */
    manuf_data[23] = bdaddr[5];
    manuf_data[24] = bdaddr[4];
    manuf_data[25] = bdaddr[3];
    manuf_data[26] = bdaddr[2];
    manuf_data[27] = bdaddr[1];
    manuf_data[28] = bdaddr[0];

    manuf_data[19] |= 0x20; /* Led */
    manuf_data[20] |= 0x05; /* Temperature and RSSI values*/
    manuf_data[20] |= 0x08; /* Battery level (percentage of full battery) */
    manuf_data[20] |= 0x10; /* Pressure value*/
    ...
    if (ret != BLE_STATUS_SUCCESS)
    {
        PRINTF("hci_le_set_scan_resp_data warning: 0x%02x\r\n", ret);
    }
    ...
    if (ret != BLE_STATUS_SUCCESS)
    {
        PRINTF("aci_gap_update_adv_data failed: 0x%02x\r\n", ret);
        return ret;
    }
